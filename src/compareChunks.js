...
function compareChunks(originalChunk, comparisonChunk) {
    return Promise.map(originalChunk, (originalPath, index) => {
        logger.info(originalPath, 'Comparing');
        let diff = new BlinkDiff({
            imageAPath: path.resolve(__dirname, '..', originalPath),
            imageBPath: path.resolve(__dirname, '..', comparisonChunk[index]),
